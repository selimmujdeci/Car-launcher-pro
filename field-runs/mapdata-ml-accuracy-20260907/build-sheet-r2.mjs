...
const FIXTURE = JSON.parse(readFileSync(
  new URL('./region2/fixture-r2.json', import.meta.url), 'utf8'));

/* ── Ölçüm parametreleri (deterministik) ─────────────────────────────────── */
const Z = 18;
const NEAR = { xmin: 34.6304, ymin: 36.8093, xmax: 34.6374, ymax: 36.8149 };  // Mersin merkez
const SEED = 20260907;            // sabit tohum — örneklem tekrar üretilebilir
const N_ML = 80;                  // Region2 hedefli örneklem (#1321)
const N_OSM = 15;                 // Region2 hedefli örneklem
const CELL = 160;                 // hücre kenarı (px) — büyük havuz için küçültüldü
...
writeFileSync(HERE + 'sheet-r2.html', html, 'utf8');

/* ── 4) Kanıt dosyaları ──────────────────────────────────────────────────── */
writeFileSync(HERE + 'mosaic-provenance-r2.json', JSON.stringify({
...
writeFileSync(HERE + 'sample-key-r2.json', JSON.stringify({
