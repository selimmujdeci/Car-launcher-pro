...
const TIMERS: readonly TimerDescriptor[] = Object.freeze([
  ...
  T({ timerId: 'mapSource.ping', owner: 'mapSourceStore', sourceRef: 'platform/mapSourceStore.ts (KALDIRILDI — F7-B)',
    timerClass: 'LEGACY_REDUNDANT', baseCadenceMs: null, minCadenceMs: null, maxCadenceMs: null,
    pressureSensitive: false, foregroundSensitive: false, criticality: 'NORMAL',
    cleanupOwner: 'detachNetworkListeners (kanonik abonelik söküm thunk’ı)', onArmWheel: false, decision: 'DELETE_IF_REDUNDANT',
    rationale: 'F7-B TAMAMLANDI: kaldırıldı. 30 sn’lik yoklama yalnız bir HTTP bağlantı PROBUNU (OSM HEAD) çalıştırıyordu; kanonik ConnectivityAuthority aynı gerçeği olay tabanlı ve PROBSUZ üretir. Bütçelenecek uyanma kalmadı — davranış (uydu/hibrit düşürme) DEĞİŞMEDİ.' }),
