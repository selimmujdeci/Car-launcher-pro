...
/** MediaStore etiketsiz dosyada sanatçı/albüm için "<unknown>" YER TUTUCUSU döner —
 *  değer değildir: ekrana ("<unknown> · Klasör") ve söz aramasına taşınmaz. */
function tagValue(v: string | null | undefined): string | null {
  return v && v.trim() && v.trim() !== '<unknown>' ? v : null;
}
function toTrack(t: LocalMusicTrack): MusicTrack { const volumeIdentity = normalizeVolumeIdentity(t.volumeName); return Object.freeze({ id: mediaTrackId(volumeIdentity, t.id), contentUri: t.uri, provenance: 'MEDIASTORE_EXTERNAL', title: t.title || null, artist: tagValue(t.artist), album: tagValue(t.album), albumArtist: tagValue(t.albumArtist), durationMs: t.durationMs > 0 ? t.durationMs : null, trackNumber: typeof t.trackNumber === 'number' && t.trackNumber > 0 ? t.trackNumber % 1000 : null, discNumber: typeof t.trackNumber === 'number' && t.trackNumber >= 1000 ? Math.floor(t.trackNumber / 1000) : (t.discNumber ?? null), folder: t.relativePath ?? null, mimeType: t.mimeType ?? null, codec: null, volumeName: t.volumeName ?? null, volumeIdentity, storageKind: t.storageKind ?? 'UNKNOWN', generationModified: t.generationModified ?? null, artworkIdentity: t.albumArtUri || null, availability: 'AVAILABLE', genre: (t.genre ?? null) || null, year: typeof t.year === 'number' && t.year > 0 ? t.year : null }); }
