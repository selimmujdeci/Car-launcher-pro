/**
 * PhoneContactsTab — Telefon Merkezi · Kişiler.
 *
 * Telefon Merkezi'nden ÖNCEKİ `PhoneScreen`in rehber gövdesi — davranış
 * BİREBİR korunur (kişi listesi, arama kutusu, favoriler, numara seçici).
 * Tek fark: arama artık paylaşılan `callNumber()` üzerinden gider ve
 * başarılı aramalar `recordCall()` ile "Son Aramalar"a (Aramalar sekmesi)
 * yazılır — önceden var olan ama hiçbir yerden çağrılmayan bir yeteneğin
 * gerçek veriyle beslenmeye başlaması.
 */
...
function ContactRow({ contact, onCall }: {
  ...
  onCall: (id: string, name: string, phones: { number: string; label: string }[]) => void;
}) {
  ...
  return (
    <button
      ...
      onClick={() => onCall(contact.id, contact.name, contact.phones)}
      ...
      <div data-editable="phone.number-picker" data-editable-type="panel" className="rounded-3xl p-6 w-full max-w-xs mx-4 shadow-2xl"
        ...
        <div className="flex flex-col gap-2.5">
          {phones.map((p) => (
            <button
              key={p.number}
              onClick={() => { onCall(p.number); onClose(); }}
...
export const PhoneContactsTab = memo(function PhoneContactsTab() {
  const { contacts, loading, error } = useContactsState();
  const [query, setQuery] = useState('');
  const [picking, setPicking] = useState<{ id: string; name: string; phones: { number: string; label: string }[] } | null>(null);
  ...
  const handleCall = (id: string, name: string, phones: { number: string; label: string }[]) => {
    if (phones.length === 0) return;
    if (phones.length === 1) { callNumber(phones[0].number, id); return; }
    setPicking({ id, name, phones });
  };

  return (
    <div data-editable="phone.contacts-tab" data-editable-type="panel"
      ...
      {picking && (
        <NumberPicker
          name={picking.name}
          phones={picking.phones}
          onCall={(number) => callNumber(number, picking.id)}
            ...
            <div className="grid grid-cols-4 gap-2">
              {favorites.slice(0, 8).map((c) => {
                ...
                return (
                  <button
                    key={c.id}
                    onClick={() => handleCall(c.id, c.name, c.phones)}
