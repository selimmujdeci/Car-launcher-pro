...
export const DrawerShell = memo(function DrawerShell({ open, onClose, children, fullscreen }: Props) {
  return (
    <div className={`fixed inset-0 z-[1000] transition-all duration-500 ${
      ...
      {/* Arka plan blur overlay — fullscreen modda daha koyu */}
      <div
        className="absolute inset-0 backdrop-blur-md"
        style={{ background: fullscreen ? 'rgba(4,8,18,0.85)' : 'rgba(4,8,18,0.65)' }}
        onClick={onClose}
      />
      ...
      <div
        style={fullscreen
          ...
          : { position: 'absolute', inset: '12px 24px 24px 24px' }}
        className={`${fullscreen ? '' : 'rounded-[40px]'} overflow-hidden flex flex-col border-none !shadow-none transition-all duration-700 ${
          open ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
        }`}
      >
        {/* Sürükle tutacağı — fullscreen modda gizli */}
        {!fullscreen && (
          <div
