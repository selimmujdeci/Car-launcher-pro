/**
 * EcuEndpointInventoryScreen — CAROS LAB · Araç · ECU Uç Noktaları & Rol Kanıtı.
 *
 * SALT-OKUNUR. Keşif KOŞTURMAZ, kimlik ÇÖZMEZ, PDU göndermez, araç bağlamı
 * aktive etmez, depoya yazmaz, timer kurmaz, ağa çıkmaz. Yalnız mevcut senkron
 * getter'ları okur.
 *
 * ── EKRANIN TEK İDDİASI ─────────────────────────────────────────────────────
 * "Bu araçta KAÇ tanı uç noktası VAR, hangisinin KİM olduğunu BİLİYORUZ ve
 *  bilmediklerimiz NEDEN bilinmiyor."
 * ADRES BULMAK ≠ ROL BİLMEK: rolsüz uç nokta bir başarısızlık değil, dürüst
 * bir ölçüm sonucudur ve envanterde KALIR.
 */

import { memo, useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { RefreshCw, ShieldCheck, Network } from 'lucide-react';
import {
  readEcuEndpointSnapshot, type EcuEndpointRawSnapshot,
} from '../../../platform/devtools/ecuEndpointSources';
import {
  buildEcuEndpointCards, countByEcuEndpointClass, deriveEcuEndpointVerdict,
  ECU_ENDPOINT_VERDICT_LABEL, type EcuEndpointVerdict,
...
const VERDICT_STYLE: Record<EcuEndpointVerdict, string> = {
  ...
  ENDPOINTS_ONLY:  'border-[var(--oem-warn)] bg-[var(--oem-warn-soft)] text-[var(--oem-warn)]',
...
const FieldRow = memo(function FieldRow(
  ...
  return (
    <div
      data-testid={`ecu-ep-field-${field.id}`}
...
export const EcuEndpointInventoryScreen = memo(function EcuEndpointInventoryScreen() {
  // Açılışta TEK okuma. TIMER YOK, ABONELİK YOK, POLLING YOK.
  const [snap, setSnap] = useState<EcuEndpointRawSnapshot>(
    () => readEcuEndpointSnapshot());
  ...
  const refresh = useCallback(() => {
    if (mountedRef.current) setSnap(readEcuEndpointSnapshot());
  }, []);

  const cards = useMemo(() => buildEcuEndpointCards(snap), [snap]);
  const verdict = useMemo(() => deriveEcuEndpointVerdict(snap), [snap]);
  const counts = useMemo(() => countByEcuEndpointClass(cards), [cards]);

  return (
    <div className="flex h-full flex-col gap-2 overflow-y-auto" data-testid="ecu-endpoint-inventory">
      <div className="shrink-0 rounded border border-[var(--oem-line)] bg-[var(--oem-surface-1)] px-3 py-2">
        <div className="flex flex-wrap items-center gap-2 font-mono text-[10px]">
          <span className="flex items-center gap-1 text-[11px] font-bold tracking-wide text-[var(--oem-info)]">
            <Network size={12} /> ECU UÇ NOKTALARI VE ROL KANITI
          </span>
        ...
        <p className="mt-1 font-mono text-[9px] leading-relaxed text-[var(--oem-ink-3)]">
          <b>ADRES BULMAK, ROL BİLMEK DEĞİLDİR.</b> Rolsüz bir uç nokta çöpe
          atılmaz; envanterde kalır ve neden çözülemediği yazılır.
          <b> KAYNAK YOK</b> ile <b>0</b> aynı hücreye yazılmaz.
        </p>
      </div>

      <div
        data-testid="ecu-ep-verdict"
      ...
      >
        ENVANTER GERÇEĞİ: {ECU_ENDPOINT_VERDICT_LABEL[verdict.status]}
        ...
        <div className="mt-1 text-[9px] opacity-50">
          Uç nokta bulunamaması, ARACIN o modülü taşımadığı anlamına GELMEZ.
        </div>
      </div>
      ...
      {cards.map((card) => (
        <div
          key={card.id}
          data-testid={`ecu-ep-card-${card.id}`}
      ...
      <p className="shrink-0 pb-2 font-mono text-[9px] leading-relaxed text-[var(--oem-ink-3)]">
        KAPSAM SINIRI (dürüstlük): aday adres uzayı adresleme ailesine göre
        KAPALIDIR — CAN11 yalnız ISO 15765-4 çifti (7E0..7E7), CAN29 ve KWP için
        standart aday uzayı YOKTUR ve uç nokta yalnız ÖLÇÜLMÜŞ responder ya da
        kaynak adresinden doğar; kör adres taraması YAPILMAZ. Uç noktaya yalnız
        rol-bağımsız SALT-OKUNUR kimlik DID okumaları gönderilir; kodlama,
        rutin, aktüatör, reset, silme ve SecurityAccess bu yoldan AÇILAMAZ.
        Ham seri, parça ve donanım numarası hiçbir kalıcı yüzeye YAZILMAZ.
        Yetenek benzerliği tek başına rol KANITI DEĞİLDİR ve ADAY seviyesini
        geçemez. Ağ geçidi topolojisi bu turda ÖLÇÜLEMEZ; gateway bypass
        YOKTUR. Rol öğrenmesi araca göre bölümlenmiştir — bir araçta öğrenilen
        rol, başka bir aracın aynı adresine UYGULANMAZ. Gerçek araç doğrulaması
...
export default EcuEndpointInventoryScreen;
