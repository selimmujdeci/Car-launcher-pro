      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-semibold text-sm text-ink">Caros <span className="text-accent-ink">Pro</span></span>
            </div>
            <p className="text-ink-3 text-sm leading-relaxed max-w-[200px]">
          ...
          {columns.map((col) => (
            <div key={col.title}>
              <p className="text-xs font-semibold tracking-widest text-ink-4 uppercase mb-4">
                {col.title}
              </p>
              <ul className="flex flex-col gap-2.5">
                {col.links.map(({ href, label }) => (
                  <li key={href}>
                    <Link href={href} className="text-sm text-ink-3 hover:text-ink transition-colors">
                      {label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="pt-8 border-t border-line flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-ink-4 text-xs">Platform Aktif</span>
          </div>
        </div>
      </div>
    </footer>
