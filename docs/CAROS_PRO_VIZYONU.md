...
- **Runtime Scheduling native kanıt körlüğü giderildi:** aynı kopya, LAB'daki poll kanıtı
  ...
  **Cihazda doğrulanmadı** (kütük #118). Durum: **ENTEGRE**.
  ...
  | UDS Gezgini · Araç Çağrısı · Kıyaslama | Ekran PLACEHOLDER | **Derin Tarama bu listeden ÇIKTI** — 2026-08-21'de açıldı (kütük #693): "iki ayrı akış" gerekçesi taramayı ÇALIŞTIRMAK içindi, GÖZLEMLEMEK için değil. Kalan üçü: `uds-explorer` istek ÜRETEN bir gezgin olduğu için güvenlik incelemesi bekliyor (bilinçli); `tool-calling` motoru (`ai/tools/toolLoop`) VAR ama hiç sayaç/durum tutmuyor → gözlemlenebilmesi için önce **motora enstrümantasyon** gerekir; `benchmark` motoru gerçekten yok — **dürüst placeholder**. |
  ...
  Katalog kapsamı bugün: **54 AVAILABLE · 3 PLACEHOLDER · 2 DISABLED** (59 araç)
  ...
  (`AVAILABLE` diyip ekranı olmayan tek araç bile yok — host kilidi mevcut; 2026-08-21 son ölçüm **54/54**).
