...
| `VITE_ENABLE_YT_DOWNLOAD` | `media/ytDownloadService.ts` (`YT_DOWNLOAD_ENABLED`), `components/media/MediaScreen.tsx` (`YT_DL`) | yok = **kapalı** | ⚠️ **SADECE dev/kişisel build.** `'true'` ise YouTube ses indirme butonu + servisi derlemeye dahil olur. **TİCARİ/RELEASE BUILD'DE ASLA SET EDİLMEZ** (YouTube ToS / telif). Bayrak yokken `MediaScreen`'deki blok + `ytDownloadService` dinamik import'u **ölü-kod olarak elenir** → satış paketinde fiziksel olarak bulunmaz. Kanıt: bayrak-kapalı build'de `dist`'te `caros-yt`/`yt-downloads`/`ytDownloadService` = 0 referans. |
