...
| **#518** | ✅ **KAPANDI (2026-08-09, gerçek araç) · P1-1 ÇÖZÜLDÜ — boru ÖLÜ DEĞİL** | ⚠️ Premis düzeltildi (P1-1 teşhisi, 2026-08-09): `samples: []` ölçümü **2026-07-15** raporundan; arka plan tüketicisini ekleyen `_watchAllSupportedPids` **2026-07-17**'de (`f1e0e7f` madde 4) eklendi ve commit'in kendisi *"🔴 Gerçek araç doğrulaması BEKLİYOR"* diyor. Kabul ölçütü: gerçek araçta **önce Runtime Scheduling → YENİLE** (yoksa `cacheState:never_refreshed` → sahte `NO_NATIVE_EVIDENCE`), sonra `extendedPollEvidence.decision` okunur ve **H1/H2/H3/H4'ten biri** kayda geçer. `H4_HEALTHY` çıkarsa P1-1 zaten kapalıdır ve #518 **kapanır**; diğer üç hüküm ayrı kütük satırı doğurur. **ÖLÇÜLDÜ VE KAPANDI (2026-08-09):** gerçek araçta (Fiat dizel, CAN 29-bit 500k, kontak açık) LAB → Sorgu Zamanlayıcı → YENİLE yapıldı; `cacheState: refreshed` · `evidenceState: native_available` · `present: true` · `evidenceComplete: true`. Hüküm: **H4 — "HAT SAĞLIKLI, native→JS→store uçtan uca akıyor"**. Destekleyen sayaçlar: `success 65` · `callbackEmitted 65` · JS köprüsü `59 olay / 0 decode hatası / 59 depolandı` · `busy 0` · `NRC 0` · `error 0` · `parseFailure 0` · bağlantı kalitesi %100. Kabul ölçütü "H4 çıkarsa #518 kapanır" diyordu → **kapandı**. **AMA aynı ölçüm ÜÇ YENİ AÇIK doğurdu** (kütük #512 extended kanal durması · #513 19→11 izleme boşluğu · #514 PID 23 NO_DATA) — H4 "boru sağlam" der, "kapsama tam" DEMEZ. |
