...
> **REVİZYON (2026-06-11, kullanıcı onaylı — AI-FIRST):** Gemini sohbeti sona
> (eski #7) değil ÖNE alındı. Kullanıcı-başlatan serbest konuşmalar companion
> açıkken ÖNCE Gemini'ye gider; keyword listeleri ana yol değil, yalnız offline
> fallback kategori ipucudur. Düşüş koşulları: internet yok · key yok ·
> hata/timeout · 429 (60sn soğuma). Komut/sohbet ayrımını voiceService router'ı
> yapar (parser ≥0.7 → komut yolu). §2.8 DEĞİŞMEDİ: proaktif konuşmalar
> Gemini'ye gitmez, şablon kalır. Persona ayrı commit olmaktan çıktı (system
> prompt satırı). Ham OBD verisi prompt'a girmez — Commit 2 yorumlayıcıları girer.

| # | Commit | İçerik | Durum |
|---|---|---|---|
| 1 | `feat(companion): settings + migration` | 7 alan, v14 migration, Ayarlar paneli | ✅ `0c478e0` |
| 2 | `feat(companion): context interpreters` | companionContext saf yorumlayıcılar + testler | ✅ `c07ac1a` |
| 3 | `feat(companion): route open conversation through Gemini` | AI-first router + Gemini sohbet + offline fallback + voice_route tanısı | ✅ |
| 4 | `feat(companion): context injection` | Trip/rota/hava yorumlarının prompt bağlamına eklenmesi (OBD yakıt/sıcaklık 3'te girdi) | — |
| 5 | `feat(companion): engine state machine` | PromptScheduler + gate zinciri + SystemBoot Wave 4 + cognitive subscribe (proaktif=şablon) | — |
| 6 | `feat(companion): wake word v2 (native)` | Grammar-kısıtlı thread, ducking'siz, half-duplex, 'wakeWord' event | — |
| 7 | `feat(companion): telemetry` | companionDiag + IncidentCenter görünürlüğü + gizlilik testleri (voice_route temeli 3'te atıldı) | — |

Her commit bağımsız çalışır durumda bırakır; 1-5 cihazsız test edilebilir, 6 K24/Duster ölçümü ister.
