-- =====================================================================
-- Migration 026: push_vehicle_event — TEXT vehicle_id şema sapması fix'i
--
-- SAHA HATASI (2026-07-06): "Tanı Gönder" (support_snapshot) hiçbir cihazdan
-- panele düşmüyordu — ama heartbeat/system_health düşüyordu. Kök neden:
--
--   Canlı public.vehicle_events.vehicle_id kolonu TEXT (elle kurulmuş şema;
--   migration 025'te get_recent_diagnostics için de keşfedildi). push_vehicle_event
--   içindeki RATE-LIMIT sorgusu bu TEXT kolonu bir UUID değişkeniyle
--   karşılaştırıyordu:
--       WHERE vehicle_id = v_vehicle_id      -- text = uuid → 42883
--   → "operator does not exist: text = uuid" → RPC İSTİSNA atıyor → event
--   EKLENMİYOR.
--
--   Rate-limit bloğu YALNIZ log-tipleri için çalışıyor
--   (critical_error/crash/log/obd_diag/support_snapshot/ota_event/voice_diag).
--   Bu yüzden log-tipleri (tanı dahil) HİÇ düşmüyordu; heartbeat/system_health
--   gibi log-DIŞI tipler bloğu atlayıp INSERT'e gittiği için düşüyordu.
--
-- FIX (minimal, sapmaya dayanıklı — migration 025 ile aynı ::text yaklaşımı):
--   rate sorgusunda v_vehicle_id::text, INSERT'te v_vehicle_id::text.
--   vehicle_locations/vehicle_telemetry kolonları UUID (log-dışı yol zaten
--   çalışıyordu) → onlar dokunulmadan UUID olarak kalır.
--
-- NOT: Canlıya Management API SQL ile uygulandı ve doğrulandı (support_snapshot
-- artık düşüyor). Bu dosya repoyu canlı ile senkron tutar (fonksiyon idempotent).
-- =====================================================================
